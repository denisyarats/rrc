...
COMMON_TRAIN_FORMAT = [('episode', 'E', 'int'), ('task_id', 'T', 'int'),
                        ('step', 'S', 'int'),
                       ('episode_reward', 'R', 'float'),
                       ('fps', 'FPS', 'float')]

COMMON_EVAL_FORMAT = [('episode', 'E', 'int'), ('task_id', 'T', 'int'),
                        ('step', 'S', 'int'),
                      ('episode_reward', 'R', 'float')]
...
AGENT_TRAIN_FORMAT = {
    ...
    'ddpg': [('batch_reward', 'BR', 'float'), ('actor_loss', 'ALOSS', 'float'),
             ...
             ('critic_q', 'Q', 'float')],
    'sacx': [('batch_reward', 'BR', 'float'), ('actor_loss', 'ALOSS', 'float'),
             ('critic_loss', 'CLOSS', 'float'), ('critic_v', 'V', 'float'),
             ('critic_q', 'Q', 'float')]
}
...
class MetersGroup(object):
    ...
    def dump(self, step, task_id, prefix, save=True):
...
class Logger(object):
    ...
    def dump(self, step, task_id, save=True, ty=None):
        step = self._update_step(step)
        if ty is None:
            self._train_mg.dump(step, task_id, 'train', save)
            self._eval_mg.dump(step, task_id, 'eval', save)
        elif ty == 'eval':
            self._eval_mg.dump(step, task_id, 'eval', save)
        elif ty == 'train':
            self._train_mg.dump(step, task_id, 'train', save)
